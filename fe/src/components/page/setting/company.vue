<template>
    <div>
        ...
        <b-alert variant="warning" show v-if="!d.ds.length">
            <h4><icon name="info"></icon> 새로 시작하셨나봐요. 회사가 하나도 없네요</h4>
            <p>회사추가를 눌러서 시작하세요</p>
        </b-alert>
            ...
            <b-card nobody 
                ...
                <b-list-group flush>
                    <b-list-group-item v-for="gr in v.gr_ids" :key="gr._id">
                        <span>{{ gr.name }}</span>
                        <b-btn-group class="float-right" size="sm">
        ...
        <b-row>
            <b-col>
                <b-btn variant="info" @click="list">새로 고침</b-btn>
    ...
    methods: {
        ...
        addGr (cp) {
            this.$sweetalert({
                ...
                .then(res => {
                    if (!res) throw new Error('')
                    return this.$axios.post(
                        `${this.$cfg.path.api}group`,
                        { cp_id: cp._id, name: res }
                    )
                })
                .then(res => {
                    if (!res.data.success) throw new Error(res.data.msg)
        ...
        delGr (gr) {
            this.$sweetalert({
                ...
                .then(res => {
                    if (!res) throw new Error('')
                    return this.$axios.delete(`${this.$cfg.path.api}group`, { params: { _id: gr._id } })
                })
                .then(res => {
                    if (!res.data.success) throw new Error(res.data.msg)
